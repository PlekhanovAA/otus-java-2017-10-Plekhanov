...
import Aleksey.Plekhanov.cache.MyCache;
...
public class DBServiceImpl implements DBService {
    ...
    private final CacheEngine<Long, UserDataSet> cache = new CacheEngineImpl<>(10,1000, 1000,false);
    ...
    public void save(UserDataSet dataSet) {
        try (Session session = sessionFactory.openSession()) {
            ...
            cache.put(new MyCache<>(dataSet.getId(), dataSet));
        }
    }

    public UserDataSet read(long id) {
        MyCache<Long, UserDataSet> cached = cache.get(id);
        if (cached != null) {
    ...
    public List<UserDataSet> readAll() {
        ...
        if (allUsers != null) {
            allUsers.forEach(user -> new MyCache<>(user.getId(), user));
        }
        return allUsers;
    }
